...
| Merged PRs | — | 38 total (#1–#51, gaps at #27–29, #32, #35–36, #38–40, #42–43, #47–49) |
...
| Email provider | Resend | Loops (audience + transactional + Watch alerts + magnet emails — templates pending) |
...
| L1+L2 validation CI | No | Yes | `validate.yml` on push — failing 6 weeks; **35/37** (null beds + 1 repeat-offender) |
| L3 smoke tests | No | Partial | Chained in ingest workflow run 29171558204 — green Jul 12 |
| External audit (Ahrefs/Clarity) | Yes | No | No new audit this week |

**Last run:** PRs #46, #50, #51 deployed Jul 6–12. **Automation opportunity:** post-deploy `smoke_test.py` in GitHub Actions; auto-update invariant lists on new state merge.
...
**Last run:** Jul 6–12 — 6/7 scheduled cron runs green; OR through 2026-07-10 (+25 batch Jul 10); MN insertDate through 2026-07-11; AZ +4 Jul 12. Jul 11 WA transient pooler failure. First 10-state run with self-healing L5b (29171558204). **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics.
...
|----------|------|-----|--------|
| P0 | Loop 4 | Fix remaining `validate.yml` failures — 34 null-bed publishable facilities; 1 repeat-offender contradiction (Marquis Oregon City) | Small |
