...
| Metric | Start (May 4) | End (Aug 16) |
|--------|---------------|--------------|
| States with frontend | CA + partial OR/WA/MN/TX | CA, OR, WA, MN, UT, IL, PA, AZ, **MO** (TX hidden) — **10 covered** |
| Merged PRs | — | 56 total (#1–#77, #87–#95; gaps at #27–29, #32, #35–#36, #38–#40, #42–#43, #47–#49, #52, #55, #61, #63, #66, #68, #74, #78–#86, #92–#93) |
...
**Last run:** Aug 10–16 — 2 nightly merges (#94–#95); OR source through 2026-08-12; MN insertDate through 2026-08-13; GHA ingested UT +2, AZ +24, PA +3 on Aug 12–13 matrix runs. No post-ingest baseline commits since Aug 8. Stale open PRs #76–#82, #93 superseded but unclosed. Pre-ingest spot-check 153/153 on #94–#95. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta; **alert when probe PRs age >24h without merge**; **auto-close superseded probe PRs when newer probe merges**; **auto-commit post-ingest baselines after successful GHA ingest**.
...
**Last run:** Aug 10–16 — Aug 16 Sunday cron (run 31935924974) in progress; per-state ingest succeeds (MN, OR, UT, CA confirmed) but **L5 post-ingest fails matrix-wide**, skipping alert dispatch on every state; admin notify also fails (Loops `submittedAt`). Nightly probe merges continue (#94–#95). Premium placement unchanged since PR #77. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization; **decouple alert dispatch from L5 failure when ingest itself succeeded**; **investigate root cause of matrix-wide L5 failures** (may be invariant drift, not per-state data).
...
| P1 | Loop 7 | **Close stale ingest probe PRs** — batch-close superseded #76–#82, #93 after #87–#95 merged | Small |
