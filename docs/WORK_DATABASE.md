...
|------|-------------|----------|--------|
| 2026-08-23 | Scheduled Weekly state source scan — Aug 23 Sunday cron queued (run pending at automation time) | Infrastructure | In progress |
...
| 2026-08-21 | Cron probe OR +9 inspections pending ingest — **PR #100 open** (superseded by #101) | Infrastructure | Blocked |
...
## Month-level progress and learning (May → mid-August 2026)

| Metric | Start (May 4) | End (Aug 23) |
|--------|---------------|--------------|
| States with frontend | CA + partial OR/WA/MN/TX | CA, OR, WA, MN, UT, IL, PA, AZ, **MO** (TX hidden) — **10 covered** |
| Merged PRs | — | 56 total (#1–#77, #87–#95; no merges Aug 14–23; gaps at #27–29, #32, #35–#36, #38–#40, #42–#43, #47–#49, #52, #55, #61, #63, #66, #68, #74, #78–#86, #92–#101) |
...
**Last run:** PR #77 deployed Jul 30 (Clearing redesign + funnel fixes). `validate.yml` failing 9th week (35/37). **Automation opportunity:** post-deploy `smoke_test.py` in GitHub Actions; auto-update invariant lists on new state merge; presentation-only design refresh checklist (Loop 14).
...
**Last run:** Aug 17–23 — no nightly merges since #95 (Aug 13); OR backlog 13 rows pending in open PR #101 (source max 2026-08-21 vs ingested 2026-08-14). Aug 16 Sunday GHA run 31935924974 ingested OR +11 and MN +16 despite L5 failure. Superseding open PR chain #98→#101. Stale PRs #76–#82, #93, #98–#100 unclosed. Pre-ingest spot-check 153/153 on #97–#101. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta; **alert when probe PRs age >24h without merge**; **auto-close superseded probe PRs when newer probe merges**; **auto-commit post-ingest baselines after successful GHA ingest**; **document that merge-to-main is the only ingest trigger** (automation env lacks `DATABASE_URL`).
...
**Last run:** Aug 16 run 31935924974 — per-state ingest completes (OR +11, MN +16) but **L5 post-ingest fails matrix-wide**, skipping alert dispatch; `notify_scan_failure.py` Loops 400 (`submittedAt` missing). Aug 23 Sunday scan queued. No probe merges Aug 14–22. Premium placement unchanged since PR #77. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization; **decouple alert dispatch from L5 failure when ingest itself succeeded**; **investigate root cause of matrix-wide L5 failures** (may be invariant drift, not per-state data).
...
|----------|------|-----|--------|
| P0 | Loop 7 | **Merge OR probe backlog** — PR #101 has 13 pending OR rows; 10-day merge gap since #95 | Owner action |
| P0 | Loop 8 | Investigate matrix-wide L5 post-ingest failures (MN/OR/UT/CA on Aug 16 scan); fix root cause or decouple alert dispatch from L5 when ingest succeeds | Medium |
...
| P1 | Loop 7 | **Close stale ingest probe PRs** — batch-close superseded #76–#82, #93, #98–#100 after #101 merges | Small |
| P1 | Loop 7 | Auto-commit post-ingest baselines after successful GHA matrix ingest (last commit Aug 8; probes through Aug 13) | Small |
