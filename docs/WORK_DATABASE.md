...
## Month-level progress and learning (May → late-August 2026)

| Metric | Start (May 4) | End (Aug 30) |
|--------|---------------|--------------|
| States with frontend | CA + partial OR/WA/MN/TX | CA, OR, WA, MN, UT, IL, PA, AZ, **MO** (TX hidden) — **10 covered** |
| Merged PRs | — | 57 total (#1–#77, #87–#95, **#106**; no merges Aug 14–25 except #106; gaps at #27–29, #32, #35–#36, #38–#40, #42–#43, #47–#49, #52, #55, #61, #63, #66, #68, #74, #78–#86, #92–#105, #107–#109) |
...
**Last run:** PR #77 deployed Jul 30 (Clearing redesign + funnel fixes). `validate.yml` failing 13th week — **31/37** (regressed from 35/37 after #106 merge; new scraper-freshness failures on MN/OR/TX/UT). **Automation opportunity:** post-deploy `smoke_test.py` in GitHub Actions; auto-update invariant lists on new state merge; presentation-only design refresh checklist (Loop 14); **tune scraper freshness thresholds per-state after matrix ingest cadence changes**.
...
**Last run:** Aug 24–30 — PR #106 merged Aug 26 (OR +5, MN insertDate +8 probe); GHA 33021970807 ingested OR +11 (max 2026-08-25), MN +5 (max 2026-08-19). Aug 23 Sunday GHA 32627736751 had already ingested OR +28, AZ +46, CA +81. Open PR #107 has OR +1, MN insertDate +3 pending. L5 fails every state post-ingest. No baseline commits since Aug 8. Stale PRs #76–#82, #93, #98–#105 unclosed. Pre-ingest spot-check 153/153 on #104–#109. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta; **alert when probe PRs age >24h without merge**; **auto-close superseded probe PRs when newer probe merges**; **auto-commit post-ingest baselines after successful GHA ingest**; **document that merge-to-main is the only ingest trigger** (automation env lacks `DATABASE_URL`).
...
**Last run:** Aug 26 GHA 33021970807 + Aug 23 GHA 32627736751 — per-state ingest completes (OR +11/+28, MN +5/+7, AZ +24/+46) but **L5 post-ingest fails matrix-wide**, skipping alert dispatch; `notify_scan_failure.py` Loops 400 (`submittedAt` missing). PR #106 merged Aug 26; open PR #107 pending. Premium placement unchanged since PR #77. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization; **decouple alert dispatch from L5 failure when ingest itself succeeded**; **investigate root cause of matrix-wide L5 failures** (may be invariant drift, not per-state data).
...
|----------|------|-----|--------|
| P0 | Loop 7 | **Merge probe PR #107** — OR +1, MN insertDate +3 pending; then commit post-ingest baselines (last commit Aug 8) | Owner action |
| P0 | Loop 8 | Investigate matrix-wide L5 post-ingest failures (Aug 23 + Aug 26 GHA runs); fix root cause or decouple alert dispatch from L5 when ingest succeeds | Medium |
...
| P1 | Loop 7 | **Close stale ingest probe PRs** — batch-close superseded #76–#82, #93, #98–#105 after #107 merges | Small |
| P1 | Loop 7 | Auto-commit post-ingest baselines after successful GHA matrix ingest (two catch-up runs Aug 23 + Aug 26; no commits since Aug 8) | Small |
