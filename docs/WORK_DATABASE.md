...
|------|-------------|----------|--------|
| 2026-09-20 | Sunday GHA weekly scan run **35511299137** queued at automation time (schedule 12:40 UTC) | Infrastructure | In progress |
| 2026-09-19 | **PR #130 merged** — cron probe OR +3 (max 2026-09-18), MN insertDate +6 (trigger weekly ingest); GHA **35475557882** (~7h14m, workflow failed; L5 post-ingest pattern unchanged) | Infrastructure | Shipped |
| 2026-09-18 | Probe baseline commit on `main` — OR max 2026-09-17, MN insertDate 2026-09-18 (staged ahead of #130 merge chain) | Infrastructure | Shipped |
| 2026-09-17 | **PR #129 merged** — cron probe OR +6, MN insertDate **+52** bulk batch (trigger weekly ingest); GHA **35405269569** (~5h18m, failed) | Infrastructure | Shipped |
| 2026-09-16 | Cron probe OR +3, MN insertDate +13 — **PR #128 open** (draft; superseded by #129–#130 merges) | Infrastructure | Shipped |
| 2026-09-15 | Cron probe — no new inspection data — **PR #127 open** (draft) | Infrastructure | Shipped |
| 2026-09-14 | Cron probe — no new inspection data — **PR #126 open** (draft) | Infrastructure | Shipped |
| 2026-09-14–20 | **Two probe PRs merged** (#129 Sep 17, #130 Sep 19) after **6-day merge gap** (#122 Sep 11 → #129); MN **+52** on #129 is largest single-week insertDate catch-up since Sep 11 +20 | Infrastructure | Shipped |
| 2026-09-14–20 | Sep 13 Sunday GHA **34758293379** completed (**failure**, ~8h) — resolves prior-week in-progress; L5 post-ingest fails; alert dispatch skipped | Infrastructure | Blocked |
| 2026-09-14–20 | Merge-triggered GHAs **35405269569**, **35475557882** (post-#129, #130) — Data Validation on push **35405269471**, **35475557828** fail in &lt;20s (31/37 pattern) | QA | Blocked |
| 2026-09-14–20 | `validate.yml` on push — **still failing** (16th week red): **31/37** unchanged; scraper freshness MN/OR/TX/UT; publishable null/zero beds; repeat-offender composite rank | QA | Blocked |
| 2026-09-14–20 | Nightly probe PRs **#125–#128** remain open drafts; superseded chain **#123–#128** plus historical **#76–#118** still unclosed | Infrastructure | Blocked |
| 2026-09-14–20 | Weekly work database — **PR #124 open** (Sep 7–13); superseded by this run on branch `cursor/work-database-loops-6e45` | Research | In progress |
| 2026-09-14–20 | **Facility Watch Premium** — operator QA (controlled $9 purchase + first concierge fulfillment) still pending | QA | In progress |
| 2026-09-14–20 | **Loops magnet templates** — 6 `LOOPS_MAGNET_*` env vars still needed before library magnet emails deliver | Deployment | Blocked |

### Week patterns (2026-09-14 → 2026-09-20)

**Dominant work types:** Infrastructure (~88%), QA (~10%), Research (~2%).

**Themes:**
1. **Loop 7 boom-bust cadence returns** — four nightly merges (#119–#122) ended Sep 11; **no merge Sep 12–16** despite probes (#126–#127 no-op, #128 staging OR +3 / MN +13). **#129** (Sep 17) lands OR +6 and MN **+52**; **#130** (Sep 19) incremental OR +3 / MN +6. Pattern: gap → compressed bulk merge → smaller follow-up.
2. **Sunday GHA + merge-triggered ingest (Loop 7/8)** — Sep 13 scan **34758293379** finished failure; #129/#130 each re-dispatched weekly scan workflows (failures). Sep 20 scan **35511299137** is next scheduled bulk run. Production freshness still tracks **merge cadence**, not probe cron alone.
3. **L5 + alert dispatch unchanged (Loop 8)** — matrix-wide L5 post-ingest failure on all merge-triggered runs; `notify_scan_failure.py` Loops `submittedAt` gap persists; subscribers see no official-record change alerts.
4. **Validation CI frozen (Loop 4)** — 16th consecutive week red at **31/37**; no remediation PRs.
5. **No new product development** — eighth consecutive ops-focused week (last feature: PR #77 Clearing, Jul 30).
6. **Stale PR debt grows** — #129–#130 merged but **#123–#128** and legacy **#76–#118** remain open; work-database docs PRs **#117, #124** still draft.

**Emerging loops:** None new. Loop 7 **bulk MN insertDate event** sub-pattern reinforced (#129 +52 after gap).

**Resolved from prior week:**
- Sep 13 GHA **34758293379** no longer in progress — completed failure (~8h).
- Sep 7–13 recovery streak did not sustain through Sep 14–16 (merge gap).

**New blockers / escalations:**
- Six-day probe merge gap (Sep 12–16) with deltas partially absorbed in #129 bulk merge; #128 still open as stale draft.
- Sep 20 Sunday GHA outcome TBD at automation time.

**Open / blocked (carried forward):**
- Batch-close stale ingest probe PRs #76–#82, #93, #98–#118 and superseded #123–#128.
...
### Month context (2026-08-20 → 2026-09-20)

Probe merges to `main`: **#106** (Aug 26) → **#113** (Sep 1) → gap → **#119–#122** (Sep 7–11 daily streak) → **6-day gap** → **#129–#130** (Sep 17, 19). Net learning: **merge streaks are fragile** — recovery bursts do not self-sustain without owner merge discipline; MN regulator posting-delay still appears as **large insertDate batches** after gaps (+20 Sep 11, +52 Sep 17). Sunday GHA and merge-triggered matrix ingest both run, but **L5 failures block Loop 8 alerts regardless**. Only feature ship in the window remains **PR #77** (Jul 30 Clearing redesign). Progress vs Sep 13: two additional merges shipped OR/MN forward; validation and alert blockers unchanged.
...
| L1+L2 validation CI | No | Yes | `validate.yml` on push — failing 16 weeks; **31/37** (scraper freshness MN/OR/TX/UT + null beds + repeat-offender composite rank) |
...
**Last run:** PR #77 deployed Jul 30 (Clearing redesign + funnel fixes). `validate.yml` failing **16th week** — **31/37** unchanged since Aug 26 regression (scraper-freshness failures on MN/OR/TX/UT; null beds; repeat-offender composite rank). Post-#129/#130 push validations **35405269471**, **35475557828** fail in seconds on merge. **Automation opportunity:** post-deploy `smoke_test.py` in GitHub Actions; auto-update invariant lists on new state merge; presentation-only design refresh checklist (Loop 14); **tune scraper freshness thresholds per-state after matrix ingest cadence changes**.
...
**Last run:** Sep 14–20 — **#129** (Sep 17: OR +6, MN insertDate +52) and **#130** (Sep 19: OR +3, MN +6) merged after 6-day gap post-#122; GHAs **35405269569**, **35475557882** failed (L5). Sep 13 GHA **34758293379** completed failure (~8h); Sep 20 **35511299137** queued. Open drafts **#123–#128** superseded but unclosed; #116/#114–#118 still open. No baseline commits since Aug 8. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta; **alert when probe PRs age >24h without merge**; **auto-close superseded probe PRs when newer probe merges**; **auto-commit post-ingest baselines after successful GHA ingest**; **document that merge-to-main is the only ingest trigger** (automation env lacks `DATABASE_URL`).
...
**Last run:** Sep 14–20 — merge-triggered GHAs post-#129/#130 (**35405269569**, **35475557882**) plus Sep 13 **34758293379** (completed failure) — per-state ingest completes but **L5 post-ingest fails matrix-wide**, skipping alert dispatch; `notify_scan_failure.py` Loops 400 (`submittedAt` missing). Premium placement unchanged since PR #77. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization; **decouple alert dispatch from L5 failure when ingest itself succeeded**; **investigate root cause of matrix-wide L5 failures** (may be invariant drift, not per-state data).
...
|----------|------|-----|--------|
| P0 | Loop 7 | **Sustain merge cadence** — keep nightly probe merges unblocked (#129–#130 shipped Sep 17/19; close superseded #123–#128); commit post-ingest baselines (last commit Aug 8) | Owner action |
...
| P1 | Loop 7 | **Close stale ingest probe PRs** — batch-close superseded #76–#82, #93, #98–#118 and #123–#128 after #130 merge | Small |
