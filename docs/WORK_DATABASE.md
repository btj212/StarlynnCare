...
## Month-level progress and learning (May → late July 2026)

| Metric | Start (May 4) | End (Aug 9) |
|--------|---------------|-------------|
| States with frontend | CA + partial OR/WA/MN/TX | CA, OR, WA, MN, UT, IL, PA, AZ, **MO** (TX hidden) — **10 covered** |
| Merged PRs | — | 54 total (#1–#77, #87–#91; gaps at #27–29, #32, #35–36, #38–40, #42–#43, #47–#49, #52, #55, #61, #63, #66, #68, #74, #78–#86) |
...
**Last run:** Aug 3–9 — merge cadence restored after Jul 27–Aug 2 backlog; 5 consecutive nightly merges (#87–#91); OR source through 2026-08-05 (+7 ingested); MN insertDate through 2026-08-08 (+2 ingested after +20/+15/+7/+5/+3 probe chain). Stale open PRs #76–#82 superseded but unclosed. Pre-ingest spot-check 153/153 on every merged PR. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta; **alert when probe PRs age >24h without merge**; **auto-close superseded probe PRs when newer probe merges**.
...
**Last run:** Aug 3–9 — Aug 9 Sunday cron dispatched matrix ingest; per-state ingest jobs succeed but **L5 post-ingest fails** on MN/IL, skipping alert dispatch; admin notify also fails (Loops `submittedAt`). Nightly probe merges restored (#87–#91). Premium placement unchanged since PR #77. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization; **decouple alert dispatch from L5 failure when ingest itself succeeded**.
...
|----------|------|-----|--------|
| P0 | Loop 8 | Weekly scan fail-soft for TX + fix `notify_scan_failure.py` Loops `submittedAt` on admin-alert template; L5 failures block alert dispatch | Small |
...
| P1 | Loop 7 | **Close stale ingest probe PRs** — batch-close superseded #76–#82 after #87–#91 merged | Small |
