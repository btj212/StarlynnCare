...
| Metric | Start (May 4) | End (Aug 2) |
|--------|---------------|-------------|
| States with frontend | CA + partial OR/WA/MN/TX | CA, OR, WA, MN, UT, IL, PA, AZ, **MO** (TX hidden) — **10 covered** |
| Merged PRs | — | 49 total (#1–#77, gaps at #27–29, #32, #35–36, #38–40, #42–43, #47–49, #52, #55, #61, #63, #66, #68, #74) |
...
**Last run:** Jul 27–Aug 2 — probes ran nightly but **6 PRs open unmerged** (#76–#82); OR +2 and MN +26 staged in #79; last production merge #73 (Jul 25). Pre-ingest spot-check 153/153 on every open PR. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta; **alert when probe PRs age >24h without merge**.
...
**Last run:** Jul 27–Aug 2 — weekly scan failed Jul 26; Aug 2 Sunday cron queued; nightly probe PRs backlogged (no matrix ingest on main since Jul 25). Premium placement shipped in PR #77. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization.
...
|----------|------|-----|--------|
| P0 | Loop 7 | **Ingest probe PR backlog** — merge or batch #76–#82; investigate why auto-merge stopped after #73 | Small |
