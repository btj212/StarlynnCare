...
| Metric | Start (May 4) | End (Jul 26) |
|--------|---------------|--------------|
| States with frontend | CA + partial OR/WA/MN/TX | CA, OR, WA, MN, UT, IL, PA, AZ, **MO** (TX hidden) — **10 covered** |
| Merged PRs | — | 48 total (#1–#73, gaps at #27–29, #32, #35–36, #38–40, #42–43, #47–49, #52, #55, #61, #63, #66, #68) |
...
**Last run:** Jul 20–26 — daily probes PRs #69–#73; OR through 2026-07-24 (+10 batch Jul 26 post-ingest); MN insertDate through 2026-07-25 (+99 catch-up Jul 23); Jul 20 no-op probe. Pre-ingest spot-check 153/153 every night. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta in probe output.
...
**Last run:** Jul 20–26 — weekly scan failing on every push/schedule (TX expected fail + Loops notify 400); Jul 26 Sunday cron queued. Nightly probe merges still trigger matrix ingest and baseline commits. Premium operator QA still pending. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization.
