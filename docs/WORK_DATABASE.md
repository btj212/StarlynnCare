...
|------|-------------|----------|--------|
| 2026-09-06 | Sunday GHA weekly scan run **34031195270** in progress — MN/OR/IL ingest complete but **L5 post-ingest fails**; alert dispatch skipped; remaining states (UT, WA, MO, TX, AZ, CA, PA) queued/in progress | Infrastructure | In progress |
...
**Last run:** Aug 31–Sep 6 — PR #113 merged Sep 1 (OR +9 max 2026-08-31, MN insertDate +3); GHA 33569498104 triggered but L5 fails every state. Sep 6 Sunday GHA 34031195270 in progress. Open PR #116 has OR +8 (max 2026-09-03), MN insertDate +8 (supersedes #114–#115). Aug 30 Sunday GHA 33313974682 current through Aug 31. No baseline commits since Aug 8. Stale PRs #76–#82, #93, #98–#111 unclosed. Pre-ingest spot-check 153/153 on #112–#116. **Automation opportunity:** alert on per-state failure; retry on `AdminShutdown`; auto-tune parallelism from pooler metrics; detect bulk catch-up vs incremental delta; **alert when probe PRs age >24h without merge**; **auto-close superseded probe PRs when newer probe merges**; **auto-commit post-ingest baselines after successful GHA ingest**; **document that merge-to-main is the only ingest trigger** (automation env lacks `DATABASE_URL`).
...
**Last run:** Sep 1 GHA 33569498104 + Sep 6 GHA 34031195270 (in progress) — per-state ingest completes but **L5 post-ingest fails matrix-wide**, skipping alert dispatch; `notify_scan_failure.py` Loops 400 (`submittedAt` missing). PR #113 merged Sep 1; open PR #116 pending (OR +8, MN insertDate +8). Premium placement unchanged since PR #77. **Automation opportunity:** fail-soft TX in workflow; fix `submittedAt` on admin-alert template; auto-retry partial scans; county-scoped dispatch generalization; **decouple alert dispatch from L5 failure when ingest itself succeeded**; **investigate root cause of matrix-wide L5 failures** (may be invariant drift, not per-state data).
