    ...
    region.state.code === "TX"
      ...
      ? [
          ...
          },
          {
            ...
            src: "PA DHS OLTL",
            delta: totalCount > 0 ? `avg ${paAvgSevere} per facility` : undefined,
          },
          {
            n: String(paTotalIj),
            label: "Immediate-jeopardy findings (severity 4) on record — highest PA DHS citation level",
            src: "PA DHS OLTL",
          },
        ]
      : [
  ...
  return (
    <>
      ...
      <div className="flex flex-col">
        ...
        <div className="border-b border-paper-rule" style={{ background: "var(--color-paper-2)" }}>
          <div className="mx-auto max-w-[1280px] px-4 sm:px-6 md:px-10 py-12">
            ...
            >
              ...
              ) : region.state.code === "PA" ? (
                <>
                  PA DHS OLTL inspection records and citation history for every licensed
                  Personal Care Home and Assisted Living Residence.
                </>
              ) : (
                <>
                ...
                <p className="font-[family-name:var(--font-display)] text-[22px] leading-[1.3] text-ink mb-4">
                  {region.state.code === "TX" ? (
                    ...
                    <>
                      Of the{" "}
                      <strong className="font-normal text-rust">{totalCount}</strong>{" "}
                      licensed PA DHS facilities indexed in {region.name},{" "}
                      <strong className="font-normal text-rust">{facilitiesWithSeriousDef}</strong>{" "}
                      ({severePct}%) have at least one severe finding (PA DHS severity ≥ 3) on
                      record.
                    </>
                  ) : (
                    <>
                  ...
                  <DataFootnote
                    ...
                    note={
                      region.state.code === "TX"
                        ...
                        : region.state.code === "PA"
                        ? "Severity ≥ 3 = serious harm potential; severity 4 = immediate jeopardy"
        ...
        {/* ── PA county: most-cited on record mini-list (both directions frame) ── */}
        {region.state.code === "PA" && isCounty && paMostCited.length > 0 && (
          <div className="border-b border-paper-rule" style={{ background: "var(--color-paper-2)" }}>
            <div className="mx-auto max-w-[1280px] px-4 sm:px-6 md:px-10 py-10">
              <p className="font-[family-name:var(--font-mono)] text-[11px] uppercase tracking-[0.14em] text-ink-4 border-t-2 border-ink pt-2.5 inline-block mb-6">
                Most-cited on record in {region.name} — PA DHS OLTL
              </p>
                ...
                {paMostCited.map((f) => (
                  <li key={f.id} className="flex items-baseline gap-3">
                    <Link
                      ...
                      className="font-[family-name:var(--font-sans)] text-[15px] text-teal underline underline-offset-2 hover:text-teal/70 transition-colors min-w-0 truncate"
                    >
                      {f.name}
                    </Link>
                    <span className="font-[family-name:var(--font-mono)] text-[12px] text-rust whitespace-nowrap shrink-0">
                      {f.serious_citations} severe finding{f.serious_citations !== 1 ? "s" : ""}
                    </span>
                    <span className="font-[family-name:var(--font-mono)] text-[11px] text-ink-4 whitespace-nowrap shrink-0">
                      {f.total_citations} total
                    </span>
                  </li>
                ))}
              </ul>
              <p className="mt-4 font-[family-name:var(--font-mono)] text-[11px] text-ink-4 tracking-[0.04em]">
                Severe = PA DHS severity ≥ 3. Full ranked list below. Source: PA DHS OLTL.
              </p>
            </div>
          </div>
        )}
