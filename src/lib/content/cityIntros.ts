...
const COUNTY_INTROS: Record<string, string[]> = {
  "CA:alameda-county": [
    ...
    "Eighty-three percent of indexed facilities in Alameda County carry at least one Type-A or Type-B deficiency in their state inspection record. That rate is not unusual by California standards, but the distribution matters: a cluster of serious citations in a single facility\u2019s history looks very different from citations spread thinly across many routine surveys. Every profile on StarlynnCare shows the full deficiency timeline, not just the most recent survey date, so you can distinguish a facility that had one bad year from one with a pattern.",
