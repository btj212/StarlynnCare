...
def fetch_target_facilities(
    ...
    *,
    ...
    if license_filter:
        ...
        params.append(pad_license_mn(license_filter))
    ...
    *,
    ...
    if dry_run:
        ...
        for block in bundle.get("facilities") or []:
            lic = pad_license_mn(block.get("license_number"))
            out.append(
                (
                    {
    ...
    by_lic = {pad_license_mn(f["license_number"]): f for f in facilities}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MN MDH inspection ingest")
